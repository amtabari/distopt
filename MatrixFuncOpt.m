function[bestfx,bestmx,bestx,e]=MatrixFuncOpt(F,D,R,TF,bldlupper,bhdhlower,Xcomp,fs_uipanel23,varb,nc,np,randomno,NOP,N,rp,...
...
% 
        ...
        for z=1:varb
            ...
            [xrandsel]=feval(@xModifiedRain,xrandsel);
            % evaluation of " x " matrix by f @handle , namely " func.m "
            for j=1:randomno%length(mxrandsel)
                fxrandsel(1,j,z)=feval(@TettaHandle,F,D,R,TF,bldlupper,bhdhlower,Xcomp,fs_uipanel23,xrandsel(:,j,z));
            end
            e=e+randomno;
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for p=1:varb
            ...
            for j=1:NOP
                ...
                xforward(p)=x(j);
                [xforward]=feval(@xModifiedRain,xforward);
                y(j)=feval(@TettaHandle,F,D,R,TF,bldlupper,bhdhlower,Xcomp,fs_uipanel23,xforward);
            end
            e=e+NOP;
            ...
            if v >= 1
                ...
                for valley=1:v
                    ...
                    for j=1:numel(xmm)
                        ...
                        xmmforward(p)=xmm(j);
                        [xmmforward]=feval(@xModifiedRain,xmmforward);
                        fxmm(j)=feval(@TettaHandle,F,D,R,TF,bldlupper,bhdhlower,Xcomp,fs_uipanel23,xmmforward);
                    end
                    e=e+ numel(xmm) ;
            ...
            [minxrain]=feval(@xModifiedRain,minxrain);
            minfxrain(p)=feval(@TettaHandle,F,D,R,TF,bldlupper,bhdhlower,Xcomp,fs_uipanel23,minxrain(:,p));
            e=e+ 1 ;
        ...
        for z=1:varb
            ...
            [xrandrain]=feval(@xModifiedRain,xrandrain);
            % evaluation of " x " matrix by f @handle , namely " func.m "
            for j=1:randomno%length(mxrandrain)
                fxrandrain(1,j,z)=feval(@TettaHandle,F,D,R,TF,bldlupper,bhdhlower,Xcomp,fs_uipanel23,xrandrain(:,j,z));
            end
            e=e+ randomno ;
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for z=1:(k)
            ...
            [xcomb]=feval(@xModifiedRain,xcomb);
            % evaluation of " x " matrix by f @handle , namely " func.m "
            for j=1:length(mxcomb)
                fxcomb(1,j,z)=feval(@TettaHandle,F,D,R,TF,bldlupper,bhdhlower,Xcomp,fs_uipanel23,xcomb(:,j,z));
            end
            e=e+ length(mxcomb) ;
        end
        %     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for z=1:varb
            ...
            [xrandcomb]=feval(@xModifiedRain,xrandcomb);
            % evaluation of " x " matrix by f @handle , namely " func.m "
            for j=1:randomno%length(mxrandcomb)
                fxrandcomb(1,j,z)=feval(@TettaHandle,F,D,R,TF,bldlupper,bhdhlower,Xcomp,fs_uipanel23,xrandcomb(:,j,z));
            end
            e=e+ randomno ;
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for z=1:(varb-1)
            ...
            [xrandsecond]=feval(@xModifiedRain,xrandsecond);
            % evaluation of " x " matrix by f @handle , namely " func.m "
            for j=1:randomno%length(mxrandsecond)
                fxrandsecond(1,j,z)=feval(@TettaHandle,F,D,R,TF,bldlupper,bhdhlower,Xcomp,fs_uipanel23,xrandsecond(:,j,z));
            end
            e=e+ randomno ;
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
